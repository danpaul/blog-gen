...
const ContentItemFile_1 = require("./ContentItemFile");
class FileMapper {
    ...
    async mapFiles() {
        return Promise.all(this.files.filter(this.filter.bind(this)).map(async (f) => await new ContentItemFile_1.ContentItemFile({
            file: f,
